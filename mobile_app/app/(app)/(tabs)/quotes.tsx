...
export default function QuotesScreen() {
  ...
  const [filter, setFilter] = useState<'All' | 'Pending' | 'Approved' | 'Rejected' | 'Liquidado'>('All');
  ...
  const filters = [
    ...
    { key: 'Approved', label: lang === 'pt' ? 'Aprovados' : 'Approved' },
    { key: 'Rejected', label: lang === 'pt' ? 'Rejeitados' : 'Rejected' },
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: palette.background }]}>
      <View style={[styles.header, { backgroundColor: palette.card, borderBottomColor: palette.border }]}>
        <Text style={[styles.title, { color: palette.text }]}>{tr(lang, 'quotes')}</Text>
        <TouchableOpacity
          style={[styles.newBtn, { backgroundColor: Colors.secondary }]}
          onPress={() => router.push('/(app)/quote/new')}
        >
          <Ionicons name="add" size={18} color="#fff" />
          <Text style={styles.newBtnText}>{tr(lang, 'newQuote')}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={[styles.filters, { borderBottomColor: palette.border }]}>
        {filters.map((f) => (
          ...
          >
            <Text style={{ color: filter === f.key ? '#fff' : palette.textSecondary, fontSize: 11, fontWeight: '600' }}>
              {f.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <DocumentListItem
            ...
            statusVariant={getQuoteVariant(item.status)}
